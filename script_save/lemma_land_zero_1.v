...
Axiom Q_land_zero :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z) (n:Numbers.BinNums.Z)
    (p:Numbers.BinNums.Z),
  ((lsl p n) = y) -> (x < (lsl 1%Z n))%Z -> ((land x y) = 0%Z).

Axiom Q_land_commutativity :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z) (a:Numbers.BinNums.Z)
    (b:Numbers.BinNums.Z),
  (x = b) -> (y = a) -> ((land x y) = (land a b)).

Axiom Q_lsl_non_zero_exists :
  forall (x:Numbers.BinNums.Z), ~ (x = 0%Z) ->
  exists i:Numbers.BinNums.Z,
  (~ ((land (-4503599627370496%Z)%Z (to_uint64 (lsl x i))) = 0%Z) /\
   (0%Z <= i)%Z) /\
  (i <= 52%Z)%Z.

Axiom Q_lsl_touint64_lsl :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z) (z:Numbers.BinNums.Z),
  (0%Z <= y)%Z -> (0%Z <= z)%Z ->
  ((to_uint64 (lsl (to_uint64 (lsl x y)) z)) = (to_uint64 (lsl x (y + z)%Z))).

Axiom Q_lsl_lsl :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z) (z:Numbers.BinNums.Z),
  (0%Z <= y)%Z -> (0%Z <= z)%Z -> ((lsl (lsl x y) z) = (lsl x (y + z)%Z)).

Axiom Q_lsl_less_then_zero :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z), (0%Z <= y)%Z ->
  (x < (lsl 1%Z y))%Z -> ((lsr x y) = 0%Z).

Axiom Q_lsl_zero_then_less :
  forall (x:Numbers.BinNums.Z) (y:Numbers.BinNums.Z), ((lsr x y) = 0%Z) ->
  (0%Z <= y)%Z -> (x < (lsl 1%Z y))%Z.
...
Axiom Q_real_2_ln_52 :
  (((Reals.Rpower.ln 4503599627370496%R) / (Reals.Rpower.ln 2%R))%R = 52%R).
...
Axiom Q_real_2_power_52 :
  ((Reals.Rpower.Rpower 2%R 52%R) = 4503599627370496%R).
...
Axiom Q_real_ln_nonneg :
  forall (x:Reals.Rdefinitions.R), (1%R <= x)%R ->
  (0%R <= (Reals.Rpower.ln x))%R.

Axiom Q_real_ln_pos :
  forall (x:Reals.Rdefinitions.R), (1%R < x)%R ->
  (0%R < (Reals.Rpower.ln x))%R.

Axiom Q_real_log_less :
  forall (x:Reals.Rdefinitions.R) (y:Reals.Rdefinitions.R), (1%R < y)%R ->
  (1%R <= x)%R -> (x < y)%R ->
  (((Reals.Rpower.ln x) / (Reals.Rpower.ln y))%R < 1%R)%R.

Axiom Q_real_log_mono :
  forall (x:Reals.Rdefinitions.R) (y:Reals.Rdefinitions.R), (0%R < x)%R ->
  (x < y)%R -> ((Reals.Rpower.ln x) < (Reals.Rpower.ln y))%R.

Axiom Q_real_log_pos :
  forall (x:Reals.Rdefinitions.R) (y:Reals.Rdefinitions.R), (1%R < y)%R ->
  (1%R <= x)%R -> (0%R <= ((Reals.Rpower.ln x) / (Reals.Rpower.ln y))%R)%R.

Axiom Q_exp_bounds :
  forall (x:t1),
  let r := t'real1 x in
  let x1 := L_exponent r in
  ~ (r = 0%R) -> ((-1023%Z)%Z <= x1)%Z /\ (x1 <= 1024%Z)%Z.

Axiom Q_make_double_from_parts :
  forall (x:Reals.Rdefinitions.R), ~ (x = 0%R) ->
  ((t'real1 (to_f64 x)) = x) ->
  ((L_make_double (L_signbit x) (L_exponent x) (L_mantissa_64bit x)) = x).
...
Theorem wp_goal :
  ...
  let x := lsl 1%Z i2 in (x = i1) -> (i < x)%Z -> ((land i1 i) = 0%Z).
(* Why3 intros i i1 i2 x h1 h2. *)
Proof.
  intros.
  rewrite Q_land_commutativity with (a := i) (b := i1); auto.
  apply Q_land_zero with (p := 1%Z) (n := i2); auto.
